...
def scrape_cookpad():
    ...
    search_url = f"{base_url}/id/cari/sehat"
    ...
    for item in recipe_items:
        ...
        # Cari semua tag gambar, baik itu <img> maupun <source> (di dalam <picture>)
        for media_tag in item.find_all(['img', 'source']):
            # Tag <img> menggunakan 'src', sedangkan <source> menggunakan 'srcset'
            # Jika 'srcset', kita ambil URL pertamanya saja (sebelum ' 1x')
            url_media = media_tag.get('src') or media_tag.get('srcset', '').split(' ')[0]
            
            # Jika URL ditemukan dan mengandung '/recipes/' (bukan '/users/'), simpan dan hentikan pencarian
            if url_media and '/recipes/' in url_media:
                gambar = url_media
                break
